...
void ADC_IRQHandler(void)
{
	...
	//ADC1->CR2 |= ADC_CR2_SWSTART; 	// 0x40000000 (1<<30) --> 연속모드로 하기 위해서는 이 구문을 지워야함
}
...
{	// ADC1: PA1(pin 41)
	...
	ADC1->CR2 &= ~(1<<1);		// CONT=0: ADC_Continuous ConvMode Disable
        //ADC1->CR2 |= (1<<1);		// CONT=1: ADC_Continuous ConvMode   --> 연속모드 사용시 메인의 while문이 안돌아가고 계속해서 ADC를 확인(Polling방식)
